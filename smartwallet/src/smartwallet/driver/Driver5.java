...
public class Driver5 {
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            if (tipe.equals("create-account")) {
            ...
            } else if (tipe.equals("show-account")) {
                String namaKerja = bagian[1];
                Account akun = daftarAkun.get(namaKerja);
                ...
                List<Transaction> daftarTransaksi = akun.getDaftarTransaksi();
                Collections.sort(daftarTransaksi, new Comparator<Transaction>() {
                    public int compare(Transaction t1, Transaction t2) {
                        return t1.getWaktu().compareTo(t2.getWaktu());
                    }
                });
                
                for (Transaction transaksi : daftarTransaksi) {
                    System.out.println(transaksi.getId() + "|" + transaksi.getTipeTransaksi() + "|" + 
                                     transaksi.getJumlah() + "|" + transaksi.getBiaya() + "|" + 
                                     transaksi.getJumlahBersih() + "|" + transaksi.getWaktu() + "|" + 
